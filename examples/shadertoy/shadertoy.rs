...
static DRAW_EVERY_FRAME: bool = true;
...
pub struct ShaderToy {
    ...
    aligned_ubo_fs: Option<Align<UboFS>>,
...
impl ShaderToy {
    ...
    fn prepare_uniform_buffers(&mut self, device: &VulkanDevice) -> VkResult<()> {
        ...
        let descriptor = vk::DescriptorBufferInfo::builder()
            .buffer(buffer)
            .offset(0)
            .range(size_of::<UboFS>() as u64)
            .build();

        self.uniform_buffer_fs = UniformBuffer {
            buffer,
            memory,
            descriptor,
        };
    ...
    fn update_uniform_buffers(&mut self, device: &VulkanDevice, camera: &Camera) -> VkResult<()> {
        ...
        unsafe {
            let data_ptr = device.logical_device.map_memory(
                self.uniform_buffer_fs.memory,
                0,
                size_of::<UboFS>() as vk::DeviceSize,
                vk::MemoryMapFlags::empty(),
            )?;
            // data_ptr.copy_from_nonoverlapping(&self.ubo_vs, 1);
            let mut aligned_ubo_fs = Align::new(
                data_ptr,
                align_of::<UboFS>() as u64,
                size_of::<UboFS>() as u64,
            );
            aligned_ubo_fs.copy_from_slice(std::slice::from_ref(&self.ubo_fs));
            self.aligned_ubo_fs = Some(aligned_ubo_fs);
            device
                .logical_device
                .unmap_memory(self.uniform_buffer_fs.memory);
        }
        Ok(())
    }
    ...
    fn setup_descriptor_set_layout(&mut self, device: &VulkanDevice) -> VkResult<()> {
        ...
        self.descriptor_set_layouts = Vec::from(descriptor_set_layouts);
    ...
    fn prepare_pipelines(
        ...
        let vertex_input_attributes = [
            // These match the following shader layout (see shadertoy.vert):
    ...
    fn setup_descriptor_set(
        ...
        self.descriptor_sets = Vec::from(descriptor_sets);
